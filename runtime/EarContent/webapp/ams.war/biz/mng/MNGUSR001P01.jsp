...
*/		
		...
		$('#j_file').bind('change', function(e, d) {
			...
			$('#MNGUSR001P01_uploadFile').Validate(function() {
				/* var request = {
						fields : {
							type_id : 'IMG'
						}
					} */
				$('#MNGUSR001P01_uploadFile').Service();
			});
		});

		$('#MNGUSR001P01_uploadFile').File({
			...
			click: function() {
				$('#allcontents').Validate(function(){
					if(confirm('저장하시겠습니까?')){
						...
						$.Service({
							transaction : 'abm.USRBBase#pUSR10005',
							request : request,
							success : function(ds) {
				...
				$.Popup({
					...
					close : function(data) {
						if (data === undefined || data === null) {
							...
							$('#EMP_ID').Value(data.HIOK_ID);
							$('#MOBILE_NO').Value(data.CELLPHONE);
							$('#EMAIL').Value(data.EMAIL);
							$('#USER_NAME').Value(data.NAME);
							$('#<%=DBNamingConstants.DEPT_CD%>').Value(<%=DBNamingConstants.DEPT_CD%>);
						}
					}
				});
			}
...
<body>
	...
	<div style="position: absolute; width: 0; height: 0; cursor: ">
		<form name='MNGUSR001P01_uploadFile' id='MNGUSR001P01_uploadFile'
						...
						<div>
							<table>
								...
								<tr>
									...
									<td colspan="3">
										<input id="EMP_ID" type="text" class="input01"/>
										<span class="button gridBtn">
											<button id="btnSSO">
												<span class="button-icon ui-icon-func-search"></span> <span
													class="button-text">사원검색</span>
											</button>
										</span>
									</td>
								...
								<tr>
									...
									<td><span id="USER_ID_DIV"></span> <input id="USER_ID" type="text" class="input01 code01" />
										<span class="button gridBtn">
											<button id="duplicateIdChk">
												<span class="button-icon ui-icon-func-approve"></span> <span
													class="button-text">중복확인</span>
											</button> </span> <input type="hidden" id="duplication_chk" value='0' /></td>
								...
								<tr>
									...
									<td>
										<div id="d_USER_GROUP">
											<select id="<%=DBNamingConstants.GROUP_ID%>"></select>
										</div>
									</td>
								...
								<tr>
									...
									<th>이메일</th>
									<td><input id="EMAIL"
										type="text" class="input01 num02" /></td>
								</tr>
								<tr>
									<th>우편번호</th>
