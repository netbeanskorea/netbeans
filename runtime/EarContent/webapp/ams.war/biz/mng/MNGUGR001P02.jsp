...
<html>
<head>
<title>사용자그룹별 권한 맵핑</title>
<%@include file="../../common/common.jsp"%>
<script>
	/*****************************************************************************************
	 * 화면 이름  : MNGUGR001P02.jsp
	 * 화면 설명  : 사용자그룹별 권한 맵핑
	 * 작성자      : 강훈기
	 * 작성일      : 2013-09-13
	 * 변경이력   :
	...
	$(function() {
		...
		$.Page({
			...
			Search_List2: function(page) {
				var request2 = $('#MNGUGR001P02').DataSet();
				...
				$.Service({
					transaction: 'abm.UGRBBase#pUGR20002',
					request: request2,
					response: '#<%=DBNamingConstants.AMS_ROLE%>',
					success: function(ds){
						$('#pager2').RecordSet(ds.recordSets.<%=DBNamingConstants.AMS_ROLE%>);
					}
				});
			}
		});
			...
			columns: [
	          	{
					...
					title: '<%=BaseUtils.getAmsLabelValue("L0026", locale, "Role Name") %>',
					align: 'left',
					width: 150
				}
			],
		...
		$('#btnMoveLeft').Button({
			click: function() {
				if(confirm('저장하시겠습니까?')){
					var selrow = $('#<%=DBNamingConstants.AMS_ROLE%>').SelectRowId();
					...
					for( var i = 0; i < len; i++) {
						var r = $('#<%=DBNamingConstants.AMS_ROLE%>').Record(selrow[i]);
...
<body>
	<div id="container">
		<div id="wrapper" class="popup clearfix">
			<div id="contentArea">
				<div class="contTitle">
					<h2><%=BaseUtils.getAmsLabelValue("L0071", locale, "Group-Role") %></h2>
				</div>
				<div class="searchArea">
					<form id="MNGUGR001P02">
						<table class="tb_search">
							...
							<tr>
								...
								<td><input id="<%=DBNamingConstants.ROLE_ID%>" class="input01" type="text" maxlength="15" /></td>
								<th><label for="<%=DBNamingConstants.ROLE_NAME%>"><%=BaseUtils.getAmsLabelValue("L0026", locale, "Role Name") %></label></th>
								<td><input id="<%=DBNamingConstants.ROLE_NAME%>" class="input01" type="text" maxlength="15" /></td>
								<td class="searchBtn">
