	...
	$(function() {
		$.Page({
			init : function() {
				...
				var id = $.Page.Data('<%=DBNamingConstants.MESSAGE_ID%>');
				var locale = $.Page.Data('<%=DBNamingConstants.LOCALE_ID%>');
				$.Service({
					transaction : 'abm.MSGBBase#pMSG10002',
					request : {
						fields : {
							<%=DBNamingConstants.MESSAGE_ID%> : id,
			...
			click : function() {
				...
				$('#form').Validate(function() {
					if (window.confirm("수정하시겠습니까?")) {
						$.Service({
							transaction : 'abm.MSGBBase#pMSG10003',
							request : request,
							success : function(ds) {
			...
			click : function() {
				...
				$('#form').Validate(function() {
					if (window.confirm("삭제하시겠습니까?")) {
						$.Service({
							transaction : 'abm.MSGBBase#pMSG10004',
							request : request,
							success : function(ds) {
								alert(ds.message.messageName);
								$.Popup.Close($.Page.Data['schData']);
