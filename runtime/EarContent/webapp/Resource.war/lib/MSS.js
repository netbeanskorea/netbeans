(function(window, $, hostIp) {
	...
	star.service.pre_process = function(setting) {
		...
		setting.request.fields.__CURR_MENU_ID__ = $.cookie('current_menu_id_web');
		...
		// 거래 시간 측정
		//alert('start');
		console.log('서비스시작 : ' + time.toLocaleTimeString() + ': ' + time.getMilliseconds() + ' ' + startTime + ' milliseconds');
		//USER AGENT 서버 전송
		setting.request.fields = setting.request.fields||{};
		setting.request.fields.__USER_AGENT__ = navigator.userAgent;
	};
	...
	star.service.call_pre_process = function(response, status, xhr) {
		...
		console.log('서비스 종료 : ' + time.toLocaleTimeString() + ': ' + time.getMilliseconds() + ' milliseconds');
		
		MssSessionLoading(response);		
		// 거래 시간 측정
	};
	 ...
	 * Page Id 추출
	 */
	$.PageIdChecker = function() {
		var location = window.location.pathname;
		var path = location.split('/');
		var length = path.length;
		var pageId = path[length - 1].split('.')[0];
	 ...
	 *     {
	 *       id: 'column1',
	 *       formatter: MSS.GridTextFormatter,
	 *       unformatter: MSS.GridTextUnformatter
	 *     },
	 *     {
	 *       id: 'column2',
	 *       formatter: MSS.GridCheckBoxFormatterFactory('y'),
	 *       unformatter: MSS.GridCheckBoxUnformatterFactory('y')
	 *     }
	 *   ],
	 *   formatter: MSS.GridTextFormatter,
	 *   unformatter: MSS.GridTextUnformatter
	 ...
	 */
	MSS.GridTextFormatter = function(value) {
		return $('<div/>').append($('<input type="text"/>').attr('value', value)).html();
	};

	MSS.GridTextUnformatter = function(cell) {
	 ...
	 */
	MSS.GridNumberFormatter = function(value) {
		return $('<div/>').append($('<input type="text" class="MSS-number"/>').attr('value', value)).html();
	};
	
	/**
	 * Grid 안에 Input Number(',' 사용) Text 삽입 
	 */
	MSS.GridNumberTextFormatter = function(value, option) {
		if(isNum(value)) {
			return $.NumberFormat({
				value: value,
				precision: ','
	...
	MSS.GridNumberUnformatter = MSS.GridTextUnformatter;

	/**
	 * Grid 안에 Input Radio 삽입
	 */
	MSS.GridRadioFormatter = function(value, option) {
		return $('<div/>').append($('<input type="radio"/>').attr('name', option.grid + '_' + option.column).attr('value', value)).html();
	};
	
	MSS.GridRadioFormatterFactory = function(initValue) {
	...
	MSS.GridRowRadioFormatter = function(value, option) {
		return $('<div/>').append($('<input id="radio" name="radio" type="radio"/>').attr('value', option.row)).html();
	};

	MSS.GridRadioUnformatter = function(cell) {
	 ...
	 */
	MSS.GridCheckBoxFormatterFactory = function(y) {
	...
	MSS.GridCheckBoxUnformatterFactory = function(y, n) {
	 ...
	 */
	MSS.HideValueformatter = function(cellvalue, options, rowObject) {
		return '<div style="text-overflow: ellipsis; white-space: nowrap; overflow: hidden;">' + cellvalue + '</div>';
	};

	MSS.HideValueUnformatter = function(cellvalue, options, cell) {
	...
	star.define('Grid', function(option) {
		...
		this.addClass('MSS-grid');
		
		this.data('MSS-option', option);
		...
		option.loadComplete = function(data) {
			...
			self.find('.MSS-number').each(function() {
				$(this).Number();
			});
			
			 // MSS 모든 그리드에서 LoadComplete시 자동 실행되어야 하는 함수
            $(this).MSSGridLoadComplete(data);
		...
		if (frozen === true) {
			var MSSOption = this.data('MSS-option');
			this.jqGrid('setFrozenColumns');
			MSSOption.MSSFrozen = true;
			this.data('MSS-option', MSSOption);
			fixPositionsOfFrozenDivs.apply(this[0], arguments);
		}
		
//		if ($(this).RecordSet().nc_list.length == 0) {
//			$(this).closest('.ui-jqgrid-bdiv').height(306);
//            $(this).EmptyRecord($(this).data().MSSOption.columns.length);
//		}
	...
	*/
    star.define('MSSGridLoadComplete', function (data) {
	 ...
	 * Grid Row 개수보기
	 */
	star.define('Counter', function(options) {
		var countOptions = options;
		var total_info = false;
		if(countOptions.total_info !== undefined)
			total_info = countOptions.total_info;
		
		var $counter = $('#' + countOptions.target); 
		var $grid = $(this);
		var tbWidth = $(this).closest('.tb_basic');
		var height;
		var width = $grid.width();
		var row;
		var counterId = $grid.attr('id') + '_counter';
		var totalId = $grid.attr('id') + '_total';
		
		// default block_size 설정
		if(countOptions.block_size == undefined)
			countOptions.block_size = [10, 20, 50, 100];
		
		// 개수보기 selectbox 구성 및 초기 설정
		if($counter.hasClass('star-counter') == false) {
			var selectHTML = '<span class="rowcnt"><select id = "' + counterId + '">';
			for(var i = 0; i < countOptions.block_size.length; i++){
				selectHTML += '<option>' + countOptions.block_size[i] + '</option>';
			}
			selectHTML += '</select></span>';
			$counter.append(selectHTML).addClass('star-counter');
			
			// 총 개수 보기 옵션 활성화
			if(total_info != false){
				var totalHTML = '<span class="brdcnt">총 건수 : <label id = "' + totalId + '"></label></span>';
		        $counter.append(totalHTML);
			}
			
			// init_size 옵션
			if(countOptions.init_size != undefined){
				row = countOptions.init_size;
				$('#' + counterId).Value(row);
			}
		}
		...
		// 총 개수 보기 옵션 활성화
		if(total_info != false)
			$('#' + totalId).text($grid.RecordSet().nc_list.length);
		
		// row 계산
		if($grid.RecordSet().nc_list.length < $('#' + counterId).Value())
			row = parseInt($grid.RecordSet().nc_list.length) + 1;
		else
			row = parseInt($('#' + counterId).Value());
		
		if($grid.RecordSet().nc_list.length !== 0)
			height = row * 25;
		else
			height = 25;

		// 가로 스크롤 설정
		if ($grid.data().MSSOption.shrinkToFit == undefined || $grid.data().MSSOption.shrinkToFit == true) {
			if($grid.hasClass('star-scroll-set') != true && $grid.height() > height) {
				$grid.addClass('star-scroll-set');
				$grid.WidthResize(width - 18);
				$grid.closest('.ui-jqgrid-bdiv').parent().find('.ui-jqgrid-hdiv').width(width);
				$grid.closest('.ui-jqgrid-bdiv').width(width);
			} else if($grid.hasClass('star-scroll-set') == true && $grid.height() <= height) {
				$grid.removeClass('star-scroll-set');
				$grid.WidthResize(width);
			} else if($grid.hasClass('star-scroll-set') == true && $grid.height() > height) {
				$grid.WidthResize(width - 18);
				$grid.closest('.ui-jqgrid-bdiv').parent().find('.ui-jqgrid-hdiv').width(width);
				$grid.closest('.ui-jqgrid-bdiv').width(width);
		...
		$grid.closest('.ui-jqgrid-bdiv').height(height);
		
		// select Change 이벤트
		$('#' + counterId).change(function() {
			var newWidth;
			var grdWidth = $grid.width();
			if(width < grdWidth && $grid.hasClass('star-scroll-set') == true && $grid.hasClass('wideGrid') == true)
				newWidth = grdWidth + 18;
			else if(width < grdWidth && $grid.hasClass('star-scroll-set') != true && $grid.hasClass('wideGrid') == true)
				newWidth = grdWidth;
			else if($grid.hasClass('wideGrid') != true)
				newWidth = width;
			
			if($grid.find('.MSS-empty').length == 1)
				return $grid.closest('.ui-jqgrid-bdiv').height(306);
			
			countOptions.init_size = $('#' + counterId).Value();
			// row 계산
			if($grid.RecordSet().nc_list.length < $('#' + counterId).Value())
				row = parseInt($grid.RecordSet().nc_list.length) + 1;
			else
			...
			if ($grid.data().MSSOption.shrinkToFit == undefined || $grid.data().MSSOption.shrinkToFit == true) {
				if($grid.hasClass('star-scroll-set') != true && $grid.height() > height) {
					$grid.addClass('star-scroll-set');
					$grid.WidthResize(newWidth - 18);
					$grid.closest('.ui-jqgrid-bdiv').parent().find('.ui-jqgrid-hdiv').width(newWidth);
					$grid.closest('.ui-jqgrid-bdiv').width(newWidth);
				} else if($grid.hasClass('star-scroll-set') == true && $grid.height() <= height) {
					$grid.removeClass('star-scroll-set');
					$grid.WidthResize(newWidth);
				} else if($grid.hasClass('star-scroll-set') == true && $grid.height() > height) {
					$grid.WidthResize(newWidth - 18);
					$grid.closest('.ui-jqgrid-bdiv').parent().find('.ui-jqgrid-hdiv').width(newWidth);
					$grid.closest('.ui-jqgrid-bdiv').width(newWidth);
	 ...
	 * Grid의 Header 정보 추출
	 */
	star.define('HeaderData', function() {
		var $grid = $(this);
		var col = $grid.getGridParam('columns');
		var length = $grid.getGridParam('columns').length;
		var colData = [];
		/*		grid: $grid.attr('id'),
				column_id: [],
				column_title: []*/
		...
		for (var i = 0; i < length; i++) {
			colData.push({
				'grid': $grid.attr('id'),
				'column_id': col[i].id,
				'column_title': col[i].title
			});
		}
		
		/*for (var i = 0; i < length; i++) {
			colData.column_id.push(col[i].id);
			colData.column_title.push(col[i].title);
		}*/
		return colData;
	});
	 ...
	 */
	star.define('SelectRowId', function(row) {
		if (!this.hasClass('MSS-grid'))
			return star.callSuper(this, arguments);
	...
	/*star.define('EmptyRecord', function (colCount) {
        var grid = $(this);
        var id = grid.attr('id');
        var message = 'No Records.';

        if ($('#' + id).find('.MSS-empty').length > 0)
            return;

		grid.jqGrid('addRowData', 'sga_' + $.jgrid.randId(), ''); // return Boolean
		
		if (!grid.hasClass('MSS-grid'))
			return star.callSuper(this, arguments);

		var tr_r = grid.find('>tbody > [role="row"]').first().siblings(); 
		
		if (tr_r.length > 1)
			grid.Clear();
		
		var tr = grid.find('> tbody > tr:last');

		if (tr.length !== 1)
			grid.Clear();
		
		tr.after('<tr role="row" id="MSS-empty_' + id + '" tabindex="-1" class="ui-widget-content jqgrow" name="emptyRecord"></tr>');

        for (var i = 0; i < colCount; i++)
            $('#MSS-empty_' + id).append('<td id="empty_' + id + '_' + i + '"></td>');

        var td = tr.children().first();
        var span = tr.children().length;
        var $empty = $('#empty_' + id + '_0');

        $empty.addClass('MSS-empty');
        $empty.attr('colspan', span).css({ 'height': '300', 'width': '' });
        $empty.text(message);

        for (var j = 1; j < colCount; j++)
            $('#empty_' + id + '_' + j).remove();

        var newWidth = $(this).closest('.tb_basic').width();
        $(this).WidthResize(newWidth);
	});*/
	...
	/*
	star.define('Add', function (addRow, dupCheckColumnIdList) {
        if (!this.hasClass('MSS-grid'))
            return star.callSuper(this, arguments);

        var grid = $(this).attr('id');

        if ($('#' + grid).find('.MSS-empty').length > 0)
            $(this).Clear();
	...
	*/
	...
	star.define('MSSSelectBoxInit', function(servicename, req, colname, reader){
		if (!this.hasClass('MSS-grid'))
			return star.callSuper(this, arguments);
		
		if(servicename===undefined || req===undefined || colname===undefined) {
			alert('Usage: MSSSelectBoxInit(ServiceName, req, Colname[, reader]');
			return ;
		}
		
		// reader 값을 설정한다
		reader = reader||{};
		reader = $.extend({rsname: 'GROUP_RS', value: 'value', label: 'label'}, reader);
	...
	star.define('HeaderGroup', function(headers, headersetFlag, test) {
		...
		if(MSSOption.MSSFrozen && MSSOption.MSSFrozen == true) {
	...
	star.define('Record', function(rowid, value) {
		if (!this.hasClass('MSS-grid-merge')) {
			return star.callSuper(this, arguments);
		}
	...
	star.define('RecordSet', function(data) {
		...
		if(data!==undefined) {
//			if (data.nc_list.length === 0) {
//				$(this).closest('.ui-jqgrid-bdiv').height(306);
//				return $(this).EmptyRecord($(this).data().MSSOption.columns.length);
//			}
	...
	// Chart Project Extend End
	
	// Calendar Project Extend Start
	star.ui.Calendar.prev_month = '<img src="../Resource/images/common/pager_pre.png"></img>';
	star.ui.Calendar.next_month = '<img src="../Resource/images/common/pager_next.png"></img>';
	star.ui.Calendar.prev_year = '<img src="../Resource/images/common/pager_first.png"></img>';
	star.ui.Calendar.next_year = '<img src="../Resource/images/common/pager_last.png"></img>';
	star.ui.Calendar.prev_month_click = function(events, currentYear, currentMonth) {
//		alert(currentYear + ' ' + currentMonth);
	};
	star.ui.Calendar.next_month_click = function(events, currentYear, currentMonth) {
//		alert(currentYear + ' ' +  currentMonth);
	};
	star.ui.Calendar.title_click = function(events, currentYear, currentMonth) {
//		alert(currentYear + ' ' +  currentMonth);
	};
	star.ui.Calendar.prev_year_click = function(events, currentYear, currentMonth) {
//		alert(currentYear + ' ' + currentMonth);
	};
	star.ui.Calendar.next_year_click = function(events, currentYear, currentMonth) {
//		alert(currentYear + ' ' +  currentMonth);
	};
	star.ui.Calendar.day_label = {
		sunday : '<label class="red">Sun</label>',
		monday : '<label>Mon</label>',
		tuesday : '<label>Tue</label>',
		wednesday : '<label>Wed</label>',
		thursday : '<label>Thu</label>',
		friday : '<label>Fri</label>',
		saturday : '<label>Sat</label>'
	}
	star.ui.Calendar.click = function(events, year, month, date, day) {
		var dayS = '';
		if(day===0) dayS = '일';
		if(day===1) dayS = '월';
		if(day===2) dayS = '화';
		...
		if(day===5) dayS = '금';
		if(day===6) dayS = '토';
		console.log('today : ' + year + '-' + month + '-' + date + '('+dayS+')');
	}
	star.ui.Calendar.data_type = 'file';
	star.ui.Calendar.data_file = prefix + '/lib/KoreaCalendar.json';
	// star.ui.Calendar.data_tr = 'com.COMBBase#pCOM10201';
	// Calendar Project Extend End
	...
	var MssSessionLoading = function(response) {
		...
		$.Session('MSSRoleInfo', response.__ROLE_INFO__);
	...
	var MenuLoading = function(menuList) {
		...
		for(var i = 0; i < menuList.length; i++) {
			if(menuList[i].PARENT_MENU_ID==='ROOT' && menuList[i].ROLE_SELECT==='Y') {
				// TOP MENU에 들어갈 거
				var menu_cls = 'gnb' + menuList[i].MENU_ID.substr(1);
				
				if(barCount2 != barCount1 - 1){
					topPage += '<li id=\"depth1_li_' + menuList[i].MENU_ID + '\">'; // class=\"on\">';
					barCount2++;
				} else
					topPage += '<li id=\"depth1_li_' + menuList[i].MENU_ID + '\" class=\"rightend\">'; // class=\"on\">';
				
				topPage += '<a href=\"javascript:depth1_click(\''+menuList[i].MENU_ID+'\', \''+ menuList[i].MENU_NAME +'\');\" class=\"' + menu_cls + '\"><em>' + menuList[i].MENU_NAME + '</em></a>';
				topPage += '</li>';
			}
		}
		topPage += '</ul>';
		...
		for(var i = 0; i < rootObject.length; i++) {
			...
			for(var j = 0; j < menuList.length; j++) {
				if(rootObject[i]===menuList[j].PARENT_MENU_ID && menuList[j].ROLE_SELECT ==='Y') {
					...
					leftMenu += '<li>';
					leftMenu += '<a class=\"depth2\" href=\"javascript:void(0);\" onclick=\"javascript:depth2_click(\''+menuList[j].MENU_ID+'\', \''+menuList[j].PAGE_URI+'\'); \"><span>' + menuList[j].MENU_NAME + '</span></a>';
					...
					for(var k = 0; k < menuList.length; k++) {
						if(leftMenuId===menuList[k].PARENT_MENU_ID && menuList[k].ROLE_SELECT ==='Y') {
							leftMenu += '<li>';
							leftMenu += '<a href=\"javascript:void(0);\" onclick=\"javascript:depth3_click(\''+menuList[k].MENU_ID+'\', \''+menuList[k].PAGE_URI+'\');\"><span>' + menuList[k].MENU_NAME + '</span></a>';
							leftMenu += '</li>';
						}
					}
	...
	/**
	 * 메뉴가 로딩되지 않을경우 메뉴를 다시 읽어오도록 함. 
	 */
	$.MenuReloading = function(location) {
		var request = {USER_ID: ''};
		console.log("Path Info : "+location);
		$.Service({
			...
			success: function(ds) {
				...
				for(var i = 0; i < options.length; i++) {
					if (options[i]){
						...
						switch(code.style) {
							case 'select':
								jDiv.html('<select id="' + code.codeId + '" name="' + code.codeId + '" class="star-codehelper" starValidLabel="' + code.codeName + '"/>');
								if(code.blank == false) {
									$('#' + code.codeId).Select({
										value: 'CODE_ID',
										label: 'CODE_NAME',
							...
							case 'radio':
								...
								for(var j = 0; j < codeList[i].length; j++) {
									jDiv.append('<input id="' + code.codeId + '" name="' + code.codeId + '" class="star-codehelper" type="radio" value="'+codeList[i][j].CODE_ID+'">'+ codeList[i][j].CODE_NAME +'</input>');
								}
								$('#' + code.codeId).Radio({
									change: code.func
								});
							...
							case 'checkbox':
								...
								for(var j = 0; j < codeList[i].length; j++) {
									var checkBoxCode = code.codeId + j;
									jDiv.append('<input id="' + checkBoxCode + '" name="' + checkBoxCode + '" class="star-codehelper" type="checkbox" >'+ codeList[i][j].CODE_NAME +'</input>');
	...
	function RowEditChange(event) {
		...
		} else if ($(this).hasClass('editEnter') && keycode == 13) {
			var grid = $(this).closest('table').attr('id');
			var grid_length = $('#' + grid).find('.jqgrow').length;
			
			var tr = $(this).closest('tr').attr('id');
			var next = parseInt(tr) + 1;
	...
	star.define('MSSRowEdit', function(row, focusColid, column, changefunc){
		if (!this.hasClass('MSS-grid'))
			return star.callSuper(this, arguments);
		...
		if(this.data('MSS-option').editEnter == true)
			this.find('input').addClass('editEnter');
	...
	star.define('MSSRowDel', function (lastsel) {
        var grid = $(this).attr('id');

        if ($('#' + grid).Column(lastsel, 'Flag') === 'R' || $('#' + grid).Column(lastsel, 'Flag') === 'U') {
            $('#' + grid).Column(lastsel, 'Flag', 'D');

            $('#' + grid).find('[title="D"]').removeClass('ui-widget-content');
            $('#' + grid).find('[title="D"]').addClass('D-row');
            $('#' + grid).find('[title="D"]').siblings().addClass('D-row');
            //$('#GRID').find('[title="D"]').css({"background-color": "#990085"});
            //$('#GRID').find('[title="D"]').siblings().css({"background-color": "#990085"});
            //$('#' + grid).find('[title="D"]').parent().Hide();

        } else if ($('#' + grid).Column(lastsel, 'Flag') === 'C') {
            $('#' + grid).Remove(lastsel);
        }
        return this;
    });
	...
	// BlockUI를 이용한 Loadoverlay
    $.ProcessMessageLoad = function (msg) {
        var message = '<ul><li><img src="'+prefix+'/images/common/ajax-loader.gif"/></li><li class="blockFont">' + '처리중입니다. 잠시만 기다려 주세요.' + '</li><ul>';

        if (typeof msg == 'string')
            message = '<ul><li><img src="'+prefix+'/images/common/ajax-loader.gif"/></li><li class="blockFont">' + msg + '</li><ul>';
