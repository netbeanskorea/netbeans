...
//		tag.child = _escapeHTML(tag.child);
	...
	_renderValue.plugin["textarea"] = function(render, value, data, mapping) {
		...
		tag.attr = $.extend(tag.attr, $.isPlainObject(render.attr) ? render.attr : null, self.option.defaultUIComponent["textarea"]["attr"]);
		if (typeof render.styleclass === "string") {
			tag.attr.styleclass += render.styleclass;
		}
		if(self.option.defaultUIComponent["textarea"]["styleclass"]) {
			tag.attr.styleclass += ' ' + self.option.defaultUIComponent["textarea"]["styleclass"];
		}
	...
	_renderValue.plugin["text"] = function(render, value, data, mapping) {
		...
		if (render.styleclass || self.option.defaultUIComponent["text"]["styleclass"]) {
			var classstring = (render.styleclass||'') + ' ' + (self.option.defaultUIComponent["text"]["styleclass"]||'');
			rendered.push('class="', classstring, '" ');
		}
		...
		var attr = render.attr;
		if(self.option.defaultUIComponent["text"]["attr"]) {
			attr = $.extend({},attr,self.option.defaultUIComponent["text"]["attr"]);
		}
		if (typeof attr == "object") {	
			for ( var prop in attr) {
				if (prop == "style") {
					styleoccured = true;
					rendered.push(prop, '="');
					if (attr[prop].toLowerCase().indexOf('width') < 0) {
						rendered.push('width:100%;');
					}
					rendered.push(attr[prop], aligntext,'" ');
				} else {
					rendered.push(prop, '="', attr[prop], '" ');
				}
			}
		}
		if(!styleoccured) {
	...
	_renderValue.plugin["select"] = function(render, value, data, mapping) {
		...
		if (render.styleclass || self.option.defaultUIComponent["select"]["styleclass"]) {
			var classstring = (render.styleclass||'') + ' ' + (self.option.defaultUIComponent["select"]["styleclass"]||'');
			rendered.push('class="', classstring, '" ');
		}
		var attr = render.attr;
		if(self.option.defaultUIComponent["select"]["attr"]) {
			attr = $.extend({}, attr, self.option.defaultUIComponent["select"]["attr"]);
		}
		if (typeof attr == "object") {
			for ( var prop in attr) {
				if (prop == "style") {
	...
	_renderValue.plugin["radio"] = function(render, value, data, mapping) {
		...
		if (typeof render.attr == "object") {
			for ( var prop in render.attr) {
				attr.push(prop, '="', render.attr[prop], '" ');
			}
		}
		if(self.option.defaultUIComponent["radio"]["attr"]){
			for ( var prop in self.option.defaultUIComponent["radio"]["attr"]) {
				attr.push(prop, '="', self.option.defaultUIComponent["radio"]["attr"][prop], '" ');
			}
		}
		attr = attr.join('');
		for ( var idx in rules) {
			...
			if (render.styleclass || self.option.defaultUIComponent["radio"]["styleclass"]) {
				var classstring = (render.styleclass||'') + ' ' + (self.option.defaultUIComponent["radio"]["styleclass"]||'');
				rendered.push('class="', classstring, '" ');
			}
			if (_ruleValue(rule, "value") == value) {
	...
	_renderValue.plugin["check"] = function(render, value, data, mapping) {
		...
		if (render.styleclass || self.option.defaultUIComponent["check"]["styleclass"]) {
			var classstring = (render.styleclass||'') + ' ' + (self.option.defaultUIComponent["check"]["styleclass"]||'');
			rendered.push('class="', classstring, '" ');
		}
		if (typeof render.attr == "object") {
			for ( var prop in render.attr) {
		...
		if(self.option.defaultUIComponent["check"]["attr"]){
			for ( var prop in self.option.defaultUIComponent["check"]["attr"]) {
				rendered.push(prop, '="', self.option.defaultUIComponent["check"]["attr"][prop], '" ');
			}
		}
	...
	$.extend(AlopexGrid, {
		...
		defaultOption: {
			...
			,defaultUIComponent : {//UI component setup which are used internally by grid.
				"check" : {},//type:"check" 또는 selector column {attr:{"data-type":"checkbox","data-wrap":"true"}}
				"radio" : {},//type:"radio"
				"select" : {},//type:"select"
				"textarea" : {},//type:"textarea"
				"text" : {}//type:"text"...{attr:{"data-type":"textinput","data-wrap":"true"}}
			}
			,eventMapping : {}
			,renderMapping : {}
			...
			,fixcolumnRowHeightCompensate:0 //if fix column is missed by 1px, set this value to 1
			...
			,overrideBrowserScroll : true//to follow browser default scroll action, set this value to false.
...
//					fixedColgroup.children('col').map(function(idx,elem){if(idx>self.state.fixupto) return elem;}).remove();
	...
	AlopexGrid.prototype._columnResizeStart = function(e, column) {
		...
		if(self.option.calculateResizingHandlePositionOldWay) {
			...
			var widthmap = self._calcCellWidth();var aa = [];
			$.each(widthmap, function(cindex, width){
				if(Number(cindex) <= Number(columnIndex)) {
					baseOffset += width;aa.push(cindex);
				}
			});
		}
...
//					m1width = e.data.target.width(); // target 넘기는 부분 주석처리해서 에러발생. 
	...
	AlopexGrid.prototype.viewEventUpdate = function(data) {
		...
		if(self.option.overrideBrowserScroll) {
				...
				var later = (function(self, $cell){
					return function(){
						...
						var tableScrollTop = (self.state.hasFixColumn && $cell.hasClass('cell-fixcol')) ? 
							(-self.state.$fixcoltable.position().top) : scrollTop;
				...
				self.state._vspTimer = setTimeout(function(){
					...
					if(vscroll && !(self.option.rowPadding && self.state.data.length < rowPaddingVal)) {
						...
						if(false && (from[1] < to[0] || to[1] <= from[0])) { //그려지는 범위에서 완전히 벗어남
							...
							self._tableSpacing(vscroll["paddingTopHeight"], vscroll["totalHeight"], vscroll["paddingBottomHeight"]);
							if(self.state.hasFixColumn) {
		...
		if(self.state.rowspanned) {
			for(var i=0,l=self.state.rowspanindex.length;i<l;i++) {
				if(self.state.rowspanindex[i]) {
					...
					while(endIndex < MAX_END_INDEX && self.state.data[endIndex][key] === self.state.data[endIndex+1][key]) {
						endIndex++;
					}
					break;
				}
	...
	AlopexGrid.prototype._cellRender = function(data, mapping, option) {
		...
		if ($.isPlainObject(mapping.attr)) {
			for ( var prop in mapping.attr) {
				...
				if($.isFunction(aval)) {
					aval = aval(data[mapping.key], data, mapping);
				}
		...
		if (mapping.numberingColumn) {
			...
			if(self.option.defaultUIComponent["check"]["attr"]) {
				$.extend(content.child.attr, self.option.defaultUIComponent["check"]["attr"]);
			}
			if(self.option.defaultUIComponent["check"]["styleclass"]) {
				content.child.attr["styleclass"] += ' '+self.option.defaultUIComponent["check"]["styleclass"];
			}
			...
			for(var ci=0;ci<headerStacks.length;ci++){
				...
				for(var ri=0;ri<headerStack.length;ri++) {
					...
					if(mapping.selectorColumn && !(typeof mapping.title === "string" && self.option.allowSelectorColumnTitle)) {
						...
						if(self.option.defaultUIComponent["check"]["attr"]) {
							$.extend(input.attr, self.option.defaultUIComponent["check"]["attr"]);
						}
						...
						if(self.option.defaultUIComponent["check"]["styleclass"]) {
							_addClassAttribute(input, self.option.defaultUIComponent["check"]["styleclass"]);
						}
					} else {
		...
		if (fixupto >= 0 && self.state.hasHorizontalScrollBar) {
			...
			if (nodata) {
				if (this.option.message && this.option.message.nodata) {
					...
					nobody.push('top:', (self.state.scrollerTopMargin+bodytopComp), 'px;left:0px;');
