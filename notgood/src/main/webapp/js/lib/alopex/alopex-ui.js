...
//		    }
		    ...
		    // popup API로 띄운 동적 마크업다이얼로그는 삭제한다.
		    if ($.alopex.util.isValid($.alopex._PopupIdList) && $.alopex._PopupIdList.length > 0){
		      var index = $.alopex._PopupIdList.indexOf(el.id);
		      if (index !== -1) {
		        $.alopex._PopupIdList.splice(index, 1);
		        $(el).remove();
		      }
		    }
		...
		_calculateHeight: function(el) {
		  ...
		  $(el).children().each(function(i, elem) {
		    ...
		    if (!$(elem).attr('data-dialog-scroll') && $(elem).css('position') !== 'absolute' 
		      && arr.indexOf(elem.nodeName.toLowerCase()) === -1) {
...
!function($) {
  ...
  if (window.opener && window.opener.$) {
    ...
    window.onbeforeunload = function() { 
      if($.alopex._AlopexPopupList && $.alopex._AlopexPopupList.length > 0) {
  ...
  if(window.opener && window.opener.$ && window.opener.$.alopex && window.opener.$.alopex._PopupConfig && window.opener.$.alopex._PopupConfig[_AlopexPopupID]) {
    ...
    if ($.alopex.util.isValid(config) && $.alopex.util.isValid(config.data)) {
      $.alopex._WindowPopupData = JSON.parse(config.data); // windowpopup data 전달.
    }
  } else if(window.parent) {
    // dialog popup PopupData setting
    $(window.parent.document).find('[data-type="dialog"]').each(function(i,elem) {
      config = $(elem).prop('alopexPopupSetting');
      if ($.alopex.util.isValid(config) && $.alopex.util.isValid(config.data) && elem.id === config.id) {
        $.alopex._WindowPopupData = JSON.parse(config.data);
      }
    });
  }
  ...
  function _GenerateId() {
    var length = $.alopex._PopupIdList.length;
    var id = 'Alopex_Popup_' + length;
    $.alopex._PopupIdList.push(id);
    return id;
  }
...
//      }
  ...
  $.extend($.alopex, {
    _PopupIdList: [], // 이 리스트의 인덱스를 활용하여 다이얼로그 id를 생성합니다.
    _AlopexPopupList: [],
    ...
    _PopupDefaultOption: {
      ...
      iframe: true,
      windowpopup: false,
      toggle: false // E&S에서 개발된 기능으로 다이얼로그 접혔다 펼쳤다하는 기능.
    },
    ...
    close: function(data) {
      ...
      if ($.alopex.util.isValid(window.opener)) {
        ...
        $(window.parent.document).find('[data-type="dialog"]').each(function(i,elem) {
          config = $(elem).prop('alopexPopupSetting');
          if ($.alopex.util.isValid(config) && elem.id === config.id) {
            // popup callback 함수 찾아서 호출
            if ($.alopex.util.isValid($(elem).prop('alopexPopupSetting').callback) && $.alopex.util.isValid(data)) {
              $(elem).prop('alopexPopupSetting').callback(data);
            }
            ...
            // dialog 닫힐때 scrollTop 위치 세팅
            var scrollTop = elem.scrollTopValue;
            ...
            var index = window.parent.$.alopex._PopupIdList.indexOf(elem.id);
            window.parent.$.alopex._PopupIdList.splice(index, 1);
            $(elem).remove();
          }
        });
      }
		...
		http.request(entity, 
		    (function(option, req){ // 이 부분은 멀티 서비스 호출 시 option 값 보존을 위해 필요.
					return function(res) {
						...
						if(req.dismissProgress) {
						  _dismissProgress(req.dismissProgress);
						}
					};
				})(option, request),
				(function(option, req) {
...
+function($) {
	...
	SimpleData.prototype.get = function(test) {
		return this._value;
	};
...
//						}
	 ...
	 * json으로 리턴한다.
	 */
	ArrayData.prototype.get = function(test) {
		var result = [];
...
//						setRule(element, rules);
	 ...
	 * 인자로 주어진 해당 스콥 내에 
	 */
	ViewModel.prototype.search = function(options) {
		var model = this;
		...
		for(var i in Controls) {
			...
			if (!options.selectOptions && i === 'options') {
			  continue;
			}
	 ...
	 * data getter 함수.
	 */
	ViewModel.prototype.get = function(test) {
		var result = {};
		for ( var i in this) {
			if (this[i] instanceof SimpleData || this[i] instanceof ArrayData || this[i] instanceof ComputedData || this[i] instanceof ObjectData) {
				result[i] = this[i].get();
	...
	Controls["options"] = {
		...
		data: function(element) {
			var ret = [];
			var optionAttr;
			var valueKey = $.alopex.databind._optionsValueKey;
			var textKey = $.alopex.databind._optionsTextKey;
...
//			data = data[key];
	...
	$.alopex.data = {
		...
		model: function() {
			...
			if(typeof arguments[0] == 'undefined') {
				...
				scope = arguments[0];
				reset = arguments[1];
				options = $.extend(true, options, arguments[2]);
			}
	...
	$.fn.getData = function(options) {
		return $.alopex.datamodel(this, true, options).get();
	};
	
	$.alopex.registerSetup('databind', function(option) {
		if(option.optionsValueKey) {
			$.alopex.databind._optionsValueKey = option.optionsValueKey;
		}
		if(option.optionsTextKey) {
			$.alopex.databind._optionsTextKey = option.optionsTextKey;
		}
	});
